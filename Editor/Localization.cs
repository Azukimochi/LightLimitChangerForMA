...
namespace io.github.azukimochi
{
    internal static class Localization
    {
        ...
        private static Dictionary<string, string[]> _LocalizedText = new Dictionary<string, string[]>()
        {
            ...
            { "label.light_max", new[] { "MaxLight[0-10]", "明るさの上限[0-10]" } },
            { "label.light_min", new[] { "MinLight[0-10]", "明るさの下限[0-10]" } },
            { "label.light_default", new[] { "DefaultLight[0-1]", "明るさの初期値[0-1]" } },
            ...
            { "label.allow_color_tmp", new[] { "Allow Color Temperature Ctrl(Advanced)", "色温度調整を有効にする(上級者向け)" } },
            { "label.allow_saturation", new[] { "Allow Saturation Control(Advanced)", "彩度調整を有効にする(上級者向け)" } },
