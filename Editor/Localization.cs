...
namespace io.github.azukimochi
{
    internal static class Localization
    {
        ...
        private static Dictionary<string, string[]> _LocalizedText = new Dictionary<string, string[]>()
        {
            ...
            { "label.use_default", new[] { "Apply on Initial State", "初期状態で適用する", "默認開啟使用" } },
            ...
            { "label.light_default", new[] { "Default Brightness", "明るさの初期値", "默認亮度" } },
            ...
            { "label.separate_light_control_init_val", new []{ "Individual Initial Value Settings", "個別の初期値設定", "個別默認值設定"}},
            { "label.light_min_default", new []{"Min Default Value", "明るさの下限の初期値", "亮度下限默認值"}},
            { "label.light_max_default", new []{ "Max Default Value", "明るさの上限の初期値", "亮度上限默認值"}},
            ...
            { "info.initial_val", new []{ "Default Values for Additional Settings", "追加設定の初期値", "追加設定的默認值" } },
            { "info.save_location", new[] { "Save Location", "アセットの保存場所", "資產保存位置" } },
            ...
            { "tip.use_default", new[] { "Use the light animation in the initial state", "初期状態でライトのアニメーションを使用します", "在默認狀態下使用亮度動畫" } },
            ...
            { "tip.light_default", new[] { "Initial brightness setting", "初期の明るさ設定", "默認的亮度設定" } },
            ...
            {
                "tip.allow_reset", new[] { "Adds a reset button to return parameters to selected values", "パラメータを設定値に戻すリセットボタンを追加します", "新增一個重置按鈕以回到默認值" }
            },
            {
